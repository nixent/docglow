...
def load_artifacts(
    ...
    Raises:
        ArtifactLoadError: If required artifacts (manifest) cannot be loaded.
    """
    ...
    if not manifest_path.exists():
        raise ArtifactLoadError(
            f"Could not find manifest.json in {resolved_target}/\n\n"
            "This file is generated by dbt. Run one of the following:\n"
            "  dbt compile    (fastest — generates manifest only)\n"
            "  dbt run        (runs models and generates manifest)\n"
            "  dbt build      (runs models, tests, and generates manifest)\n\n"
            "If your dbt artifacts are in a different location, use:\n"
            "  docglow generate --target-dir /path/to/target/"
        )
    manifest_data = _load_json(manifest_path)
    manifest = Manifest.model_validate(manifest_data)
    _log_artifact_info("manifest.json", manifest.metadata.dbt_schema_version)

    # Optional: catalog.json (degrades gracefully)
    catalog_data = _load_optional_json(resolved_target / "catalog.json", "catalog.json")
    if catalog_data is not None:
        catalog = Catalog.model_validate(catalog_data)
        _log_artifact_info("catalog.json", catalog.metadata.dbt_schema_version)
    else:
        logger.warning(
            "catalog.json not found — column types will be unavailable. "
            "Run 'dbt docs generate' to include column type information."
        )
        catalog = Catalog()
